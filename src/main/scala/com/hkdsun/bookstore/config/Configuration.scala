...
trait Configuration {
  ...
  lazy val rootDirectory = Try(homedir + '/' + config.getString("discovery.rootDir")).getOrElse(homedir + "/.mybookstore")
  ...
  lazy val retryInterval = Try(config.getInt("discovery.retry-interval")).getOrElse(500)
  lazy val retryLimit = Try(config.getInt("discovery.retry-limit")).getOrElse(5)
}
